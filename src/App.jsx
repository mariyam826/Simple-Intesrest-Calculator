...
function App() {
  ...
  const validateUserInputs = (e) => {
    ...
    //  console.log(!!value.match(/^[0-9]*.?[0-9]+$/));//!! to make boolean
    if (!!value.match(/^\d*\.?\d+$/)) {
      //valid pattern
      if (name === 'principle') {
        setPrinciple(value)
  ...
  return (
    <div style={{ width: '100%', height: '100vh' }} className='d-flex  justify-content-center align-items-center bg-dark'>
      <div style={{ width: '50rem' }} className='bg-light p-5 rounded'>
        ...
        <form className='mt-5' onSubmit={handleCalculate}>
          {/* material UI   - import from component api*/}
          <div className="mb-3">
        ...
        {/* stack instead of flex in react */}
        <Stack direction={'row'} spacing={2}>
          <Button type='submit' className='w-50 bg-dark' variant="contained" disabled={validPrinciple && validRate && validYear ? false : true} >CALCULATE</Button>
          <Button className='w-50 text-dark' style={{ height: "7vh" }} variant="outlined" onClick={handleReset} >RESET</Button>
        </Stack>
        </form>
      </div>
    </div>
